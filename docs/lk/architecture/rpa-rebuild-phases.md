...
## Runtime through Phase 2q validate (when env-gated); Phase 2p remains **planning + manifest** for validate (probe imports **2p** for **2q** evidence; **worker** must not import **2p**)
...
**Phase 2p has no `MLCC_ADD_BY_CODE_PHASE_2P` env flag.** [`mlcc-phase-2p-policy.js`](../../../services/api/src/workers/mlcc-phase-2p-policy.js) is **imported by the probe only** (for **2q** evidence). The **worker** must **not** import **2p** (`verify:lk:rpa-safety`).
...
- **`buildPhase2pPostValidateLadder()`** — `validate_order_bounded_interaction` is **`implemented_as_phase_2q_when_env_gated`**; later steps remain out of scope until separate phases.
- **Truthfulness:** Do **not** assume MLCC validate is generally safe from one run; **2p** + **2q** disclaimers forbid over-claiming backend or checkout truth.
...
## Next execution phase (2r — planning placeholder)

**Post-validate read-only observation** as its own extended phase (if needed beyond **2q**’s optional settle scrape), or **checkout/submit** planning only — **no** checkout/submit/finalize in **2r** unless a future doc + verify + policy bump explicitly authorizes it.
