...
## Runtime through Phase 2n (as implemented for single add/apply-line click when gated); Phase 2m remains canonical gate manifest (probe-only import)
...
| **2m** | **Canonical gate manifest** for add/apply-line and post-add/apply ladder — **no** standalone env flag. **Probe** imports [`mlcc-phase-2m-policy.js`](../../../services/api/src/workers/mlcc-phase-2m-policy.js) to echo `buildPhase2mAddApplyLineFutureGateManifest()` during **Phase 2n** evidence; **worker must not** import **2m** (`verify:lk:rpa-safety`) | Read policy file; run `verify:lk:rpa-safety` | Worker importing **2m**; claiming validate/checkout/submit readiness from **2m** alone |
...
**Phase 2m does not have its own browser entry point** and has **no** `MLCC_ADD_BY_CODE_PHASE_2M` env flag. [`mlcc-phase-2m-policy.js`](../../../services/api/src/workers/mlcc-phase-2m-policy.js) is **imported by the probe only** (for **2n** evidence). The **worker** must **not** import **2m** (`verify:lk:rpa-safety`).
...
- **Post-add/apply ladder:** `buildPhase2mPostAddApplyLadder()` — first step **`implemented_as_phase_2n_when_env_gated`**; `post_add_apply_observation` → `validate_order` → `checkout_submit` remain **`out_of_scope_until_separate_approval`** until implemented.
...
## Next execution phase (2o — not implemented)

**Post-add/apply read-only observation** (aligned with ladder step `post_add_apply_observation`): DOM/network guard tally **without** further mutation clicks, still **no** validate/checkout/submit until a future phase.
