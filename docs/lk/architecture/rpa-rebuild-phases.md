...
## Completed through Phase 2d (as implemented)
...
| **2d** | Mutation-boundary map: read-only scan of visible controls; heuristic `safe` / `unsafe_mutation_likely` / `uncertain` | `MLCC_ADD_BY_CODE_PHASE_2D=true` (requires 2b probe) | Any click in 2d scan, typing, mutation |
...
- `MLCC_ADD_BY_CODE_PROBE`, `MLCC_ADD_BY_CODE_PHASE_2C`, `MLCC_ADD_BY_CODE_PHASE_2D`
...
## Next phase target (2e — not implemented as code in this doc)

- Optional **scoped** boundary scan (e.g. root selector for add-by-code panel only).
- Operator-driven allowlist / tenant labels for `uncertain` controls — still **no** cart mutation until a **separate approved** phase.
