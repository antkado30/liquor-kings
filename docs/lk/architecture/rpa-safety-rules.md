...
1. **Code paths** — Worker does not implement submit/checkout/validate/add-to-cart flows. **Phase 2n** is the only path that may click add-line / apply-line (env-gated, at most once). **Phase 2o** performs **no** clicks — read-only observation after a successful **2n** click only. Success notes distinguish combined rehearsal clears, **2n**, and **2o**. `assertMlccSubmissionAllowed` exists for **future** submit code and **throws** unless `MLCC_SUBMISSION_ARMED=true` (currently unused in the happy path).
...
- Phase 2c/2d/2e/2f/2g/2h/2j/2l/2n/2o errors are wrapped with identifiable messages (`MLCC add-by-code phase 2c failed`, `… phase 2h failed`, `… phase 2j failed`, `… phase 2l failed`, `… phase 2n failed`, `… phase 2o failed`, etc.) for operator diagnostics.
...
- **Ladder after 2l:** **2n** implements the single add/apply-line click when env-gated; **2o** implements read-only post-click observation when env-gated; validate → checkout/submit remain **`out_of_scope_until_separate_approval`**; **Phase 2m** is the canonical manifest (probe import only; see below).
