...
1. **Code paths** — Worker does not implement submit/checkout/validate/add-to-cart flows. **Phase 2n** is the only path that may click add-line / apply-line (env-gated, at most once); it does **not** implement validate/checkout/submit. Success notes distinguish combined rehearsal clears from **2n** click semantics. `assertMlccSubmissionAllowed` exists for **future** submit code and **throws** unless `MLCC_SUBMISSION_ARMED=true` (currently unused in the happy path).
...
- Phase 2c/2d/2e/2f/2g/2h/2j/2l/2n errors are wrapped with identifiable messages (`MLCC add-by-code phase 2c failed`, `… phase 2h failed`, `… phase 2j failed`, `… phase 2l failed`, `… phase 2n failed`, etc.) for operator diagnostics.
...
- **Ladder after 2l:** **2n** implements the single add/apply-line click when env-gated; validate → checkout/submit remain **`out_of_scope_until_separate_approval`**; **Phase 2m** is the canonical manifest (probe import only; see below).

## Add/apply-line manifest (Phase 2m) and runtime click (Phase 2n)

[`services/api/src/workers/mlcc-phase-2m-policy.js`](../../../services/api/src/workers/mlcc-phase-2m-policy.js) exports `buildPhase2mAddApplyLineFutureGateManifest` and `buildPhase2mPostAddApplyLadder`. The **probe** imports **2m** to echo the manifest during **Phase 2n**; the **worker** must **not** import **2m** (verify enforces).

- **Criteria (2m / 2n):** successful **2l** in the same run; tenant-listed add/apply control selector(s) (JSON array); **at most one** click; Layer 2/3 expectations and hard-fail stops as in the manifest; mandatory disclaimers that browser evidence and zero abort counts **do not** prove server cart state or readiness for validate/checkout/submit.
- **Ladder:** `add_apply_line_rehearsal` is **implemented** as **2n** when env-gated; post-add **read-only** observation → validate → checkout/submit remain **`out_of_scope_until_separate_approval`** until explicitly implemented.

## Drift enforcement

- Run `npm run verify:lk:rpa-safety` from repo root (includes Phase **2i**, **2k**, and **2m** policy files, phases-doc markers, Phase **2l**/**2n** probe/worker markers, probe import of **2k** and **2m**, guard that the **worker** does not import **2k** or **2m**).
