 ...
 * - MLCC_ADD_BY_CODE_PHASE_2L_TEST_CODE — same rules as Phase 2h test code (non-empty, ≤64 chars, no newlines)
 ...
 * Phase 2o (read-only post-add/apply observation after 2n; no clicks; no validate/checkout/submit):
  ...
  if (addByCodePhase2o) {
    if (!addByCodeProbe) {
      errors.push({
        type: "config",
        message:
          "MLCC_ADD_BY_CODE_PHASE_2O=true requires MLCC_ADD_BY_CODE_PROBE=true",
      });
    }
    ...
    if (!addByCodePhase2oApproved) {
      errors.push({
        type: "config",
        message:
          "MLCC_ADD_BY_CODE_PHASE_2O=true requires MLCC_ADD_BY_CODE_PHASE_2O_APPROVED=true",
      });
    }
  ...
  try {
    ...
    evidenceCollected.push(
      await buildStepEvidence({
        ...
        message:
          "Final ordering-ready checkpoint after Phase 2b/2c/2d|2e/2f/2g/2h/2j/2l/2n/2o/2q/2r (2o/2r read-only when enabled; 2q at most one validate click when enabled; no checkout/submit/finalize)",
        kind: "mlcc_step_snapshot",
        buildEvidence,
        config,
      }),
    ...
    await finalizeRun({
      ...
      evidence: [
        ...
        buildEvidence({
          ...
          stage: "completed",
          message:
            "MLCC browser dry-run completed (Phase 2a/2b/2c/2d|2e/2f/2g/2h/2j/2l/2n/2o/2q/2r checkpoints; 2n at most one add/apply click when enabled; 2o read-only after add/apply when enabled; 2q at most one validate click when enabled; 2r read-only post-validate when enabled; no checkout/submit/finalize)",
          attributes: {
            finalUrl,
            title,
