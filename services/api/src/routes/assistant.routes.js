 ...
 * in lib/assistant.js, returns a grounded answer.
 *
...
/**
 * POST /assistant/ask
 * Body: { question: string, storeId?: string }
 * 200 → { answer, toolCalls, model, iterations }
 * 400 → { error } when question missing
...
router.post("/ask", async (req, res) => {
  ...
  try {
    const result = await askAssistant({ question, storeId });
