...
| `GET /operator-review/api/diagnostics/overview` | Operator session required. Store-scoped execution aggregates + recent `lk_system_diagnostics` (this store + `store_id` null) + **`queue_health`** (live queued/running snapshot: ages, stale heartbeats, inferred stuck counts, worker_id / heartbeat pulse) + **`trends`** (24h hourly / 7d & 30d daily UTC series). Thresholds: `QUEUE_HEALTH_THRESHOLDS` in `operator-diagnostics.service.js`. Optional query: `days`, `diag_limit`, `run_limit`. |
